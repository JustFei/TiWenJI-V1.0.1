...
-(void)babyDelegate1{
    ...
    [babycao setBlockOnDisconnectAtChannel:channelOnPeropheralView block:^(CBCentralManager *central, CBPeripheral *peripheral, NSError *error) {
        ...
        //重连方法
        [weakSelf reconnectMethod:peripheral];
    }];
...
//                设置高温一秒后push
                ...
                if (notification != nil) {
                    ...
                    _Gdaoji = [NSTimer scheduledTimerWithTimeInterval:time * 60 target:self selector:@selector(Dbaojing) userInfo:nil repeats:NO];
...
- (IBAction)connectPer:(UIButton *)sender {
    ...
    vc.disconnetCallBack = ^void(CBPeripheral * peripheral)
    {
        [_Ddaoji invalidate];
        _Ddaoji = nil;
        [_Gdaoji invalidate];
